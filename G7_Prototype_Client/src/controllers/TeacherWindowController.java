...
public class TeacherWindowController implements Initializable, IScreenController {
	...
	public void addNewQuestion(ActionEvent event) {
		...
		if (subjectComboBox.getValue() == null) {
			Utilities.popUpMethod("Please Select Subject");
			return;
		}
	...
	public void createExamInit(ActionEvent event) {
		...
		if (subject == null) {
			Utilities.popUpMethod("You must select the subject first");
			return;
		}
		String course = courseInCreateExamComboBox.getValue();
		if (course == null) {
			Utilities.popUpMethod("Please select for which course you creating the exam");
			return;
		}
		try {
	...
	public void changeTimeButtonHandler(ActionEvent event) {
		...
		okButton.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				// Exam selectedExam =
				// tableViewInExamsManagement.getSelectionModel().getSelectedItem();
				// // check executionCode
				// ActiveExam activeExam = new ActiveExam(selectedExam,
				// executionCode.getText());
				// client.handleMessageFromClientUI(new ActiveExamHandle("ChangeTime",
				// activeExam));
				// //tableViewInExamsManagement.getItems().clear();
				// Utilities.popUpMethod("Exam activated successfully!");
				primaryStage.hide();
